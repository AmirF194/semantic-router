...
class ModelCatalogSnapshotTests(unittest.TestCase):
    def _built_in_root(self, temporary: str) -> Path:
        ...
        shutil.copytree(REPO_ROOT / "config" / "built-in" / "latest", root / "latest")
        return root
    ...
    def test_snapshot_is_a_release_bound_exact_five_projection(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("config/built-in/v9.8/chorus-v1/config.yaml", probes)
            self.assertNotIn("config/built-in/latest/chorus-v1/config.yaml", probes)
            metadata_text = (bundle / "metadata.yaml").read_text(encoding="utf-8")
            metadata = yaml.safe_load(metadata_text)
            self.assertTrue(
                metadata["links"]["source"].endswith("/config/built-in/v9.8/chorus-v1")
            )
            self.assertTrue(
                metadata["links"]["documentation"].endswith(
                    "/config/built-in/v9.8/chorus-v1/README.md"
                )
            )
            for name in MODEL_BUNDLE_FILES:
                self.assertNotIn(
                    "config/built-in/latest/chorus-v1",
    ...
    def test_published_tag_snapshot_is_immutable_but_new_version_is_allowed(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            built_in_root = repo / "config" / "built-in"
