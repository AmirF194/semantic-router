...
class RecipeDistributionWorkflowTests(unittest.TestCase):
    ...
    def test_workflow_validates_manually_and_after_catalog_merges(self) -> None:
        ...
        self.assertIn("config/built-in/**", push["paths"])
        self.assertIn("config/recipes/**", push["paths"])
        self.assertIn("src/vllm-sr/cli/commands/model.py", push["paths"])
        self.assertIn("src/vllm-sr/cli/model_assets/**", push["paths"])
        self.assertIn("tools/release/sync_model_catalog.py", push["paths"])
        self.assertIn("tools/release/snapshot_model_catalog.py", push["paths"])
    ...
    def test_workflow_keeps_only_a_short_lived_validation_receipt(self) -> None:
        ...
        self.assertIn("find config/built-in src/vllm-sr/cli/model_assets", self.text)
