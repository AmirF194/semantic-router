...
BUILT_IN_ROOT = REPO_ROOT / "config" / "built-in"
...
def expected_release_files(latest_dir: Path, snapshot: str) -> dict[Path, bytes]:
    ...
    for bundle in bundles:
        ...
        latest_prefix = f"config/built-in/latest/{bundle}".encode()
        release_prefix = f"config/built-in/{snapshot}/{bundle}".encode()
...
def release_snapshot_errors(built_in_root: Path, snapshot: str) -> list[str]:
    ...
    if not release_dir.is_dir():
        return [f"missing release catalog snapshot: config/built-in/{snapshot}"]
