...
describe('Recipe API client', () => {
  ...
  it('imports without activating and sends the optional pinned archive digest', async () => {
    const response = { id: 'secure', recipe_digest: 'sha256:recipe', warnings: [] }
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/recipe/import', {
      method: 'POST',
  ...
  it('preserves machine-readable package errors for safe UI classification', async () => {
    vi.stubGlobal(
      ...
      vi.fn(
        async () =>
          new Response(JSON.stringify({ error: 'archive_digest_mismatch', message: 'mismatch' }), {
            status: 422,
          }),
      ),
    )

    const request = importRecipePackage({ url: 'https://example.com/secure.zip' })
