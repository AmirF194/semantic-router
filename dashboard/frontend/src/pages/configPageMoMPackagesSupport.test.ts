...
describe('Recipe package management support', () => {
  ...
  it.each([
    [
      ...
      true,
      {
        canImport: false,
        canActivate: true,
        importBlocker: 'permission_required',
    ...
    [
      ...
      true,
      {
        canImport: false,
        canActivate: true,
        importBlocker: 'recipe_store_readonly',
        activationBlocker: null,
      },
    ],
  ] as const)(
    'separates package capabilities (serverReadonly=%s, runtimeWritable=%s, storeWritable=%s, write=%s, deploy=%s)',
    (serverReadonly, runtimeWritable, storeWritable, canWrite, canDeploy, expected) => {
      expect(
        resolveRecipePackageCapabilities(
          serverReadonly,
          runtimeWritable,
  ...
  it('normalizes a public HTTPS package URL and pinned SHA-256', () => {
    const result = validateRecipePackageImport(
      '  https://github.com/vllm-project/semantic-router/releases/download/v1/recipe.zip?token=secret#ignored  ',
      ` SHA256:${'A'.repeat(64)} `,
    )
    ...
    expect(result).toEqual({
      ok: true,
      request: {
        url: 'https://github.com/vllm-project/semantic-router/releases/download/v1/recipe.zip?token=secret',
        expected_archive_sha256: `sha256:${'a'.repeat(64)}`,
      },
    })
  })

  it.each([
    ['http://example.com/recipe.zip', '', 'url'],
    ['https://user:secret@example.com/recipe.zip', '', 'url'],
    ['https://example.com/recipe.zip', 'not-a-digest', 'sha256'],
  ])('rejects unsafe or malformed import input', (url, sha256, field) => {
    expect(validateRecipePackageImport(url, sha256)).toMatchObject({ ok: false, field })
  })

  it.each([
    ['download_failed', 'Package download failed'],
    ['source_forbidden', 'Package source was blocked'],
    ['invalid_recipe', 'Recipe schema validation failed'],
    ['package_version_conflict', 'Package version conflict'],
    ...
    ['recipe_store_readonly', 'Recipe package store is read-only'],
    ...
    const error = new RecipeApiError(
      'https://user:secret@example.com/recipe.zip?token=private',
      422,
      code,
    )

    const presentation = presentRecipePackageError(error, 'import')
    expect(presentation.title).toBe(title)
    expect(`${presentation.title} ${presentation.message}`).not.toContain('example.com')
    expect(`${presentation.title} ${presentation.message}`).not.toContain('private')
  })
