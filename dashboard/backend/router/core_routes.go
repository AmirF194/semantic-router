...
func registerCoreRoutes(mux *http.ServeMux, cfg *config.Config, stores ...*recipe.Store) {
	store := selectedRecipeStore(cfg, stores)
	registerHealthAndSetupRoutes(mux, cfg)
	registerConfigRoutes(mux, cfg, store)
...
func registerConfigRoutes(mux *http.ServeMux, cfg *config.Config, credentialProvider ...*recipe.Store) {
	runtimeConfigReadonly := cfg.ReadonlyMode || !cfg.RuntimeConfigWritable
