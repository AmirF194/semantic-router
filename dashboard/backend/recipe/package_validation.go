...
func validateRuntimeConfig(data []byte) error {
	if err := rejectYAMLIndirection(data); err != nil {
		return err
	}
...
func rejectYAMLIndirection(data []byte) error {
	...
	walk = func(node *yaml.Node) error {
		...
		if node.Style&yaml.TaggedStyle != 0 || node.Anchor != "" || node.Alias != nil || node.Kind == yaml.AliasNode ||
			...
			return errors.New("config.yaml must not contain YAML anchors, aliases, merge keys, or explicit tags")
		}
