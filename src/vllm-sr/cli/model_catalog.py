...
def load_model_catalog(version: str = DEFAULT_CHANNEL) -> ModelCatalog:
    version, document = _load_catalog_document(version)
    header = _parse_catalog_header(document, version)
    models = _parse_catalog_models(document, version, header)
...
def load_all_model_catalogs() -> tuple[ModelCatalog, ...]:
    return tuple(
        load_model_catalog(version) for version in available_catalog_versions()
    )


def find_catalog_model(
    model_id: str, *, catalog_version: str = DEFAULT_CHANNEL
...
def catalog_model_to_dict(model: CatalogModel) -> dict[str, Any]:
    return {
        ...
        "verification": "verified" if model.verified else "unverified",
    }
