...
_CREDENTIAL_KEY_PATTERN = re.compile(
    r"(?:^|_)(?:api_?key|access_?key|client_?secret|password|secret|token|private_key)$"
)
...
def _credential_paths(data: bytes) -> tuple[str, ...]:
    try:
    ...
    def is_literal_credential(key: str, parent_key: str, value: object) -> bool:
        ...
        return not (
            ...
            and _PURE_CREDENTIAL_ENV_REFERENCE_PATTERN.fullmatch(value.strip())
        )

    def visit(value: object, path: str, parent_key: str = "") -> None:
        if isinstance(value, dict):
            for key, child in value.items():
                key_text = str(key)
                child_path = f"{path}.{key_text}" if path else key_text
                if is_literal_credential(
                    key_text, parent_key, child
                ) or _contains_embedded_url_credential(child):
                    found.add(child_path)
                visit(child, child_path, key_text.strip().lower().replace("-", "_"))
...
def _contains_embedded_url_credential(value: object) -> bool:
    ...
    try:
        ...
        if parsed.username is not None:
            return True
        return any(
            _credential_query_key(key) for key, _value in parse_qsl(parsed.query)
        )
    except ValueError:
        return False
...
def pack_recipe(recipe_dir: Path, output: Path | None = None) -> PackageResult:
    ...
    credential_paths = _credential_paths(files["config.yaml"])
    if credential_paths:
        raise RecipePackageError(
